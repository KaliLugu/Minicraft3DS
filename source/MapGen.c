...
void createAndValidateTopMap(int w, int h, uByte level, uByte *map, uByte *data) {
    do {
        ...
        for (i = 0; i < w * h; ++i)
            count[map[i] & 0xff]++;
        if (count[TILE_ROCK & 0xff] < 100)
            continue;
        if (count[TILE_SAND & 0xff] < 100)
            continue;
        if (count[TILE_GRASS & 0xff] < 100)
            continue;
        if (count[TILE_TREE & 0xff] < 100)
            continue;
        if (count[TILE_STAIRS_DOWN & 0xff] < 2)
            continue;
...
void createAndValidateUndergroundMap(int w, int h, int depthLevel, uByte level, uByte *map, uByte *data) {
    do {
        ...
        if (count[TILE_ROCK & 0xff] < 100)
            continue;
        if (count[TILE_DIRT & 0xff] < 100)
            continue;
        switch (depthLevel) {
        case 1:
            if (count[TILE_IRONORE & 0xff] < 20)
                continue;
            break;
        case 2:
            if (count[TILE_GOLDORE & 0xff] < 20 || count[TILE_MYCELIUM & 0xff] < 40)
                continue;
            break;
        case 3:
            if (count[TILE_GEMORE & 0xff] < 20)
                continue;
            break;
        }
        if (depthLevel < 3)
...
void createAndValidateDungeonMap(int w, int h, uByte level, uByte *map, uByte *data) {
    do {
        ...
        for (i = 0; i < w * h; ++i)
            count[map[i] & 0xff]++;
        if (count[TILE_DUNGEON_WALL & 0xff] < 100)
            continue;
        if (count[TILE_DUNGEON_FLOOR & 0xff] < 100)
            continue;
...
void createVillageHouse(int hid, int x, int y, int hw, int hh, int ex, int ey, int w, int h, uByte level, uByte *map, uByte *data) {
    ...
    for (xp = x; xp < x + hw; ++xp) {
        for (yp = y; yp < y + hh; ++yp) {
            if (xp == x || xp == x + hw - 1 || yp == y || yp == y + hh - 1)
                map[xp + yp * w] = TILE_WOOD_WALL;
            else
                map[xp + yp * w] = TILE_WOOD_FLOOR;
        }
    }
    ...
    if (hid == 2)
        loffx = 12;
    addEntityToList(newEntityFurniture(getIdFromName("ITEM_LANTERN"), NULL, ((x + hw / 2) << 4) + loffx, ((y + hh / 2) << 4) + loffy, level), &eManager);
}
...
void createDwarfHouse(int w, int h, uByte level, uByte *map, uByte *data) {
    ...
    for (xp = x; xp < x + vw; ++xp) {
        for (yp = y; yp < y + vh; ++yp) {
            if (xp == x || xp == x + vw - 1 || yp == y || yp == y + vh - 1)
                map[xp + yp * w] = TILE_STONE_WALL;
            else
                map[xp + yp * w] = TILE_DIRT;
        }
    }
    ...
    addEntityToList(newEntityFurniture(getIdFromName("ITEM_LANTERN"), NULL, (x + 1 + 1) << 4, (y + 1 + 3) << 4, level), &eManager);
    addEntityToList(newEntityFurniture(getIdFromName("ITEM_LANTERN"), NULL, (x + vw - 1 - 1) << 4, (y + 1 + 3) << 4, level), &eManager);
}
...
void createDungeonRoom(int w, int h, bool dragon, uByte level, uByte *map, uByte *data) {
    ...
    for (tries = 0; tries < 100; ++tries) {
        ...
        while ((checkForFloor && (map[i] != TILE_DUNGEON_FLOOR && map[i] != 255)) || (!checkForFloor && (map[i] == TILE_DUNGEON_FLOOR || map[i] == 255))) {
            ...
            if (!checkForFloor && (map[i] != TILE_DUNGEON_FLOOR && map[i] != 255))
                checkForFloor = true;
        }
