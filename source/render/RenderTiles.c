...
static void renderDots(int x, int y, u8 bits1, u8 bits2, u8 bits3, u8 bits4, u32 xTile, u32 yTile) {
    ...
    if (tu && tl)
        tileRender8(x, y, xTile, yTile, bits1);
    if (tu && tr)
        tileRender8(x + 8, y, xTile + 8, yTile, bits2);
    if (td && tl)
        tileRender8(x, y + 8, xTile, yTile + 8, bits3);
    if (td && tr)
        tileRender8(x + 8, y + 8, xTile + 8, yTile + 8, bits4);
}
...
void renderTile(int i, int d, u8 level, int x, int y) {
    ...
    case TILE_GRASS:
        ...
        if (level == 1 && worldData.season == 3)
            ...
            renderConnectedTile4(x, y, 256, 128);
        else
    ...
    case TILE_FLOWER:
        renderTile(TILE_GRASS, 0, level, x, y);
        if (level == 1 && worldData.season == 3)
            tileRender16(x, y, 320, 112, d);
        else
            tileRender16(x, y, 320, 48, d);
        break;
    ...
    case TILE_MYCELIUM:
        ...
        if (level == 1 && worldData.season == 3)
            ...
            renderConnectedTile4(x, y, 448, 80);
        break;
